...
export function mapPagBankMovement(item: PagBankEdiMovement): ProviderMovement {
  ...
  if (!movementId || !externalSaleId || !method || !date || !Number.isFinite(gross) || gross <= 0) {
    return {
      providerMovementId: movementId || `${externalSaleId ?? "unknown"}-${date}`,
      externalSaleId,
      externalEventCode: eventCode,
      kind: "SALE",
      sale: null,
      ...
      rejectionMessage: "Venda sem identificador, data, valor ou meio de pagamento valido",
    };
  }
  ...
  return {
    ...
    sale: {
      ...
      grossAmount: gross,
      netAmount: Number(item.valor_liquido_transacao ?? gross - fee),
      feeAmount: fee,
