...
describe("MercadoPagoReconciliationService", () => {
  ...
  it("persists financial operations as non-sales during reconciliation", async () => {
    const prisma: any = { salesIntegration: { update: vi.fn() } };
    const client: any = {
      searchPayments: vi
        .fn()
        .mockResolvedValue([
          { ...mercadoPagoApprovedPaymentFixture, operation_type: "money_transfer" },
        ]),
    };
    const authenticated: any = { execute: vi.fn(({ request }) => request("token")) };
    const states: any = { upsertFromMovement: vi.fn() };
    ...
    await service.reconcileConnection("tenant", "integration", 24);
  ...
  it("pages connections and reconciles them sequentially", async () => {
    ...
    expect(prisma.salesIntegration.findMany).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({ cursor: { id: "i-24" }, skip: 1, take: 25 })
    );
  ...
  it("uses the same active key for short and daily durable reconciliation", async () => {
    ...
    const prisma: any = {
      salesIntegration: {
        findMany: vi.fn().mockResolvedValue([{ id: "integration", tenantId: "tenant" }]),
      },
    };
    const scheduler = new MercadoPagoReconciliationScheduler(
      ...
      prisma,
      {
        get: vi.fn((key: string) =>
          key === "MP_RECONCILIATION_DURABLE_JOBS_ENABLED" ? "true" : "25"
        ),
      } as any,
      { enqueue } as any
    );
    ...
    expect(enqueue.mock.calls.map(([input]) => input.dedupeKey)).toEqual([
      "integration",
      "integration",
    ]);
