...
import type { FinancialAccount, FinancialAuditRecord, Payable, PayablePaymentInput } from "@burgoos/types";
...
export function PayableDetailDialog({
  ...
  async function submitReverse(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const formData = new FormData(event.currentTarget);
    await onReversePayment(String(formData.get("paymentId") ?? ""), String(formData.get("reason") ?? ""));
  }
    ...
    >
      <div className="space-y-5 text-sm text-slate-700">
        ...
        {payable.status !== "CANCELLED" && payable.status !== "PAID" ? (
          <form className="grid gap-3 rounded-md border border-slate-200 p-3" onSubmit={submitPayment}>
              ...
              payable.payments.map((payment) => (
                ...
                >
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <span>
                      R$ {payment.amount} em {formatDate(payment.paidAt)} - {payment.financialAccountName}
                    </span>
        ...
        {!payable.cancelledAt && payable.paidAmount === "0.00" ? (
          <form className="grid gap-2 rounded-md border border-red-100 bg-red-50 p-3" onSubmit={submitCancel}>
