...
export function SalesRunSummary({
  ...
  return (
    <div className="rounded border bg-slate-50 p-4 text-sm" aria-live="polite">
      ...
      <p className="mt-1">
        Encontradas {run.counts.found} · novas {run.counts.new} · duplicadas {run.counts.duplicate}{" "}
        · rejeitadas {run.counts.rejected} · falhas {run.counts.failed} · dias bloqueados{" "}
        {run.counts.blockedDays}
      </p>
      {run.days?.map((day) => (
        <p key={day.date} className="text-slate-600">
          {day.date}: {day.status} ({day.pagesFetched}/{day.totalPages ?? 0} paginas)
          {day.errorMessage ? ` · ${day.errorMessage}` : ""}
        </p>
      ))}
...
export function SalesImportHistory({ history }: { history: SalesImportRunView[] }) {
  if (history.length === 0)
    return <p className="text-sm text-slate-500">Nenhuma execucao encontrada.</p>;
  return (
    <details>
      <summary className="cursor-pointer font-semibold">Historico de execucoes</summary>
...
export function SalesIntegrationPanel({
  token,
  products,
}: {
  token: string;
  ...
  async function refresh() {
    const [integrations, runs, catalog] = await Promise.all([
      listSalesIntegrations(token),
      listSalesImportRuns(token),
      listSalesProviders(token),
    ]);
  ...
  useEffect(() => {
    void refresh().catch((error: unknown) =>
      setMessage(error instanceof Error ? error.message : "Falha ao carregar integracao")
    );
  ...
  async function configure(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage("Integracao PagBank ativa. O token permanece oculto.");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Falha ao configurar");
    } finally {
      setBusy(false);
    }
  }
    ...
    try {
      ...
      setMessage(
        ["PREVIEW_READY", "PARTIALLY_READY"].includes(current.status)
          ? "Pre-visualizacao pronta para conferencia."
          : (current.errorMessage ?? "Consulta finalizada sem datas prontas.")
      );
      await refresh();
    } catch (error) {
  ...
  async function confirm() {
    ...
    try {
      ...
      let updated = await getSalesImportRun(token, run.id);
      for (
        let attempt = 0;
        attempt < 60 && ["PREVIEW_READY", "PARTIALLY_READY", "IMPORTING"].includes(updated.status);
        attempt += 1
      ) {
        await new Promise((resolve) => setTimeout(resolve, 1000));
        updated = await getSalesImportRun(token, run.id);
        setRun(updated);
      }
      setRun(updated);
      setMessage(
        `Importacao concluida: ${updated.counts.imported} importadas, ${updated.counts.duplicate} duplicadas e ${updated.counts.failed} falhas.`
      );
      await refresh();
    } catch (error) {
  ...
  return (
    <section className="mt-8 grid gap-5 rounded-md border border-slate-200 bg-white p-5 shadow-sm">
      {selectedProvider === "PAGBANK" ? (
        <div>
          <h2 className="text-xl font-semibold">Integracao PagBank EDI</h2>
          <p className="mt-1 text-sm text-slate-600">
            Consulta movimentos transacionais consolidados de D-1. Tentativas nao autorizadas,
            vouchers e recargas nao aparecem no EDI.
          </p>
        </div>
      ) : null}
      ...
      {selectedProvider === "MERCADO_PAGO" ? (
        ...
        <>
          <form className="grid gap-3 md:grid-cols-3" onSubmit={configure}>
            ...
            >
              {integration?.status === "ACTIVE" ? "Atualizar integracao" : "Salvar e ativar"}
            </button>
          </form>
          ...
          {run ? (
            <div className="rounded border bg-slate-50 p-4 text-sm">
              <p className="font-semibold">Execucao {run.status}</p>
              <p className="mt-1">
                Encontradas {run.counts.found} · novas {run.counts.new} · duplicadas{" "}
                {run.counts.duplicate} · rejeitadas {run.counts.rejected} · dias bloqueados{" "}
                {run.counts.blockedDays}
              </p>
              {run.days?.map((day) => (
                <p key={day.date} className="text-slate-600">
